...
fn open_play_close_symbor() {
    ...
    let rust_i32_mut: &mut i32 =
        unsafe { lib.reference_mut_cstr(c"rust_i32_mut") }.unwrap();
    assert_eq!(42, *rust_i32_mut);
    *rust_i32_mut = 55; //should not crash
                        //for a change use pointer to obtain its value
    let rust_i32_ptr =
        unsafe { lib.symbol_cstr::<*const i32>(c"rust_i32_mut") }.unwrap();
    ...
    let c_struct: &SomeData =
        unsafe { lib.reference_cstr(c"c_struct") }.unwrap();
    ...
    let c_const_char_ptr =
        unsafe { lib.symbol_cstr::<*const c_char>(c"c_const_char_ptr") }
            .unwrap();
